...
import { Box, Grid, TextField, Button, Dialog, DialogContent, DialogActions, DialogContentText, DialogTitle } from '@material-ui/core';
...
function DoctorLogin() {
    ...
    function login(e) {
        ...
        if (error === 0) {
            ...
            axios.post(config.baseURL + "/doctor/login/", formdata, {
                headers: {
            ...
            <Grid container item xs={12} sm={12} md={12} lg={12} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(45deg, #EEE 30%, #DDD 90%)', minHeight: '92vh' }}>
                <Grid container item spacing={3} xs={10} sm={6} md={4} lg={4} style={{ background: '#ffffff', borderRadius: '5px' }}>
                    ...
                    <Grid item xs={12} sm={12} md={12} lg={12} fullWidth style={{ textAlign: 'center' }}>
                        <Link to='/doctor/register' style={{ textDecoration: 'none', color: '#17b0ab', fontWeight: 'bold' }}>Not yet registered? Register here</Link>
                    </Grid>
                </Grid>
            </Grid>
        </Grid>
